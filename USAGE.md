...
```bash
$corgias profiling -h
usage: corgias profiling [-h] -m {naive,rle,cwa,asa,cotr,sev} [-og OG_TABLE] [-a ASR_FOLDER] -o OUTPUT [-t TREE] [-c CORES] [--ignore_branch] [--gpu]  [-nb NUM_BLOCKS] [--test TEST] [--legacy_n]
...
| `--test TEST`           | Number of orthologs to process in test mode.
| `--legacy_n`             | `sev` only: use the old dataset-wide constant N (number of internal nodes in the tree) instead of the per-pair active-node-union N (see Output below). Ignored for other methods. |
...
- **N**: For `cotr`, the number of genomes (constant across all pairs). For `sev` (default), a **per-pair** value: the union of the internal nodes where OG1 and/or OG2 has changed at least once since the root — this excludes clades where neither OG ever gains nor loses, so N shrinks for pairs confined to a small part of the tree instead of always equalling the full tree size. Pass `--legacy_n` at the `profiling` step to fall back to the old dataset-wide constant (number of internal nodes in the tree) for all pairs.


## Statictical test
The `stat` subcommand performs statistical tests on the results generated by `profiling`.
```
corgias stat [-h] -i INPUT [-i2 INPUT2] -m {naive,rle,cwa,asa,cotr,sev} [-o OUTPUT] [-d {both,correlation,anti-correlation}] [-c CORES] [-t THRETHOLD] [-s {bonferroni,sidak,holm-sidak,simes-hochberg,hommel,fdr_bh,fdr_by,fdr_tsbh,fdr_tsbky}] [--only_signif] [--min_k MIN_K] [--min_phi MIN_PHI]
...
|-------------------------|-----------------------------------------------------------------------------|
| `-h, --help`            | Show the help message and exit.                                            |
| `-i, --input INPUT`     | Input file (primary stat result when `-i2` is used, otherwise raw profiling result). |
| `-i2, --input2 INPUT2`  | Secondary profiling result (raw) for combined correction with primary stat result (`-i`). |
...
| `--min_k MIN_K`         | `cotr`/`sev` only: keep only pairs with at least `MIN_K` co-transition events (`\|k\|`). Applied after multiple testing correction. |
| `--min_phi MIN_PHI`     | Keep only pairs with `\|phi\| >= MIN_PHI` (effect size, independent of dataset size — see Output below). Applied after multiple testing correction. |

### Combined correction with secondary profiling results (`-i2`)

When secondary profiling is run with `-og2`/`-a2`, use `-i2` to test the secondary pairs and apply a single multiple testing correction across all primary + secondary pairs. `-i` must be the output of a previous `corgias stat` run (not a raw profiling result).

```bash
# Step 1: primary profiling and stat
corgias profiling -m naive -og primary.csv -o primary_profiling.csv -c 4
corgias stat -i primary_profiling.csv -m naive -o primary_stat.csv -c 4

# Step 2: secondary profiling (cross + secondary all-vs-all)
corgias profiling -m naive -og primary.csv -og2 secondary.csv -o secondary_profiling.csv -c 4

# Step 3: combined stat — tests secondary pairs and applies joint multiple testing correction
corgias stat -i primary_stat.csv -i2 secondary_profiling.csv -m naive -o combined_stat.csv -c 4
```

**Note:** `-i` must contain all primary pairs (do not use `--only_signif` when generating the primary stat for combined correction).

### Output
The output CSV file from the `stat` subcommand contains:

| OG1   | OG2   | odds / direction | phi | pvalue | qvalue | signif |
|-------|-------|------|-----|--------|--------|--------|
| OG0001| OG0002| 2.5/10  | 0.42 | 0.01   | 0.02   | True   |

- **odds / direction**: Odds ratio (weighted methods) / Difference of concordance and discordance changes (transtion methods)
- **phi**: Phi coefficient (Matthews correlation coefficient), an effect size for the pair's 2×2 contingency table, in [-1, 1]. Unlike the p/q-value, it does not shrink as the dataset size (number of genomes or tree size) grows, so it stays comparable across pairs and datasets. With very large datasets, q-value alone can flag pairs with a negligible real association (q≈0 but phi≈0) — filtering on `--min_phi` in addition to q-value is recommended in that case.
